...
#[test]
fn test_to_pkcs8() {
    for alg in [&ec::P521, &ec::P384, &ec::P256] {
        let key_pair_doc = ec::KeyPair::generate(alg).unwrap().to_pkcs8v1().unwrap();
        let key_pair = ec::KeyPair::from_pkcs8(alg, key_pair_doc.as_ref()).unwrap();
...
fn test_private_key() {
    for alg in [&ec::P521, &ec::P384, &ec::P256] {
        ...
        let key_pair_copy = ec::KeyPair::from_private_key_and_public_key(
            alg,
            private_key.as_ref(),
            public_key.as_ref(),
        )
